...
dba <- rnc(read("2b_a"), "tcnv")
dbb <- rnc(read("2b_b"), "tcnv")

dca <- rnc(tcv, 3.5, var="day", fml=kill~day)
dcb <- rnc(trg, 3.5, var="day", fml=kill~day)
dcc <- rnc(d2c, "tcnv")

d4c <- rnc(read("4c"), "high")
d4d <- rnc(read("4d"), "high")
d4e <- rnc(read("4e"), "high")
d4f <- rnc(read("4f"), "high")
